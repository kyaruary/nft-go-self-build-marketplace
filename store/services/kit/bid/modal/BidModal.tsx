import React, { ReactElement, useEffect, useState, useRef, Dispatch, SetStateAction, useContext } from 'react';
...
import { BidModalRenderer, BidStep, BidData, Trait, BidModalStepData } from './BidModalRenderer';
import TokenStats from './TokenStats';
import dayjs from 'dayjs';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCalendar, faClose, faChevronDown, faCheckCircle } from '@fortawesome/free-solid-svg-icons';
...
export function BidModal({
  ...
  const [localMarketplace, setLocalMarketplace] = useState<ReturnType<typeof getLocalMarketplaceData> | null>(null);
  ...
  return (
    ...
    >
      {({
        ...
        const itemImage = token && token.token?.imageSmall ? token.token?.imageSmall : (collection?.image as string);

        const quantityEnabled = !tokenId || (token?.token?.kind === 'erc1155' && Number(token?.token?.supply) > 1);
        ...
        useEffect(() => {
          if (expirationOption && expirationOption.relativeTime) {
            const newExpirationTime = expirationOption.relativeTimeUnit
              ? dayjs().add(expirationOption.relativeTime, expirationOption.relativeTimeUnit)
        ...
        return (
          <Modal
            ...
            onPointerDownOutside={(e: any) => {
              if (
                ...
                datetimeElement.current.flatpickr.calendarContainer.contains(e.target)
              ) {
                e.preventDefault();
              }
            }}
          ...
          >
            {bidStep === BidStep.SetPrice && collection && (
              ...
              >
                <TokenStats token={token ? token : undefined} collection={collection} trait={trait} />
                <MainContainer css={{ p: '$4' }}>
                  <Flex justify="between">
                    <Text style="tiny" color="subtle">
                      Offer Amount {quantityEnabled ? 'and Quantity' : null}
                    </Text>
                    <Text as={Flex} css={{ gap: '$1' }} align="center" style="tiny">
                      Balance:{' '}
                      <FormatWrappedCurrency
                        logoWidth={10}
                        textStyle="tiny"
                  ...
                  <Flex css={{ mt: '$2', gap: quantityEnabled ? '$2' : 20 }}>
                    <Text as={Flex} css={{ gap: '$2', '@bp1': { ml: '$3' }, flexShrink: 0 }} align="center" style="body1" color="subtle">
                      {currencies.length > 1 ? (
                        <CurrencySelector currency={currency} currencies={currencies} setCurrency={setCurrency} triggerCss={{ width: 90 }} />
                      ) : (
                        <>
                          <CryptoCurrencyIcon css={{ height: 20 }} address={wrappedContractAddress} />
                          {wrappedContractName}
                        </>
                      )}
                    </Text>
                    ...
                    {quantityEnabled ? (
                      <Flex css={{ display: 'none', '@bp1': { display: 'flex' } }}>
                        <QuantitySelector
                          quantity={quantity}
                  ...
                  {quantityEnabled ? (
                    ...
                    >
                      <Text style="subtitle2" color="subtle" css={{ width: 90, flexShrink: 0 }}>
                        Quantity
                      </Text>
                      <QuantitySelector
                        quantity={quantity}
                  ...
                  {quantityEnabled ? (
                    <Flex align="center" css={{ gap: '$2', mt: '$3', mb: '$4' }}>
                      ...
                      />
                      <FormatCurrency style="tiny" color="subtle" amount={totalBidAmountUsd} />
                    </Flex>
                  ) : (
                    <FormatCurrency
                      css={{
                  ...
                  {attributes && attributes.length > 0 && (attributesSelectable || trait) && !tokenId && (
                    <>
                      <Text as={Box} css={{ mb: '$2' }} style="tiny" color="subtle">
                        Attributes
                      </Text>
                      <Popover.Root open={attributeSelectorOpen} onOpenChange={attributesSelectable ? setAttributeSelectorOpen : undefined}>
                              ...
                              {trait ? (
                                <>
                                  ...
                                  >
                                    {trait?.floorAskPrice && (
                                      <Box css={{ flex: 'none' }}>
                                        <FormatCryptoCurrency amount={trait?.floorAskPrice} maximumFractionDigits={2} logoWidth={11} />
                                      </Box>
                                    )}
                                    <FontAwesomeIcon
                                      style={{
                        ...
                        <Popover.Content sideOffset={-50}>
                          <AttributeSelector attributes={attributes} tokenCount={tokenCount} setTrait={setTrait} setOpen={setAttributeSelectorOpen} />
                        </Popover.Content>
                      </Popover.Root>
                    </>
                  )}

                  <Text as={Box} css={{ mt: '$4', mb: '$2' }} style="tiny" color="subtle">
                    Expiration Date
                  </Text>
                  <Flex css={{ gap: '$2', mb: '$4' }}>
                    <Select
                    ...
                    <DateInput
                      ref={datetimeElement}
                      icon={<FontAwesomeIcon icon={faCalendar} width={14} height={16} />}
                  ...
                  <Box css={{ width: '100%', mt: 'auto' }}>
                    {oracleEnabled && (
                      ...
                      >
                        You can change or cancel your offer for free on {localMarketplace?.title}.
                      </Text>
                    )}
                    {bidAmountPerUnit === '' && (
                      ...
                      <>
                        {!hasEnoughNativeCurrency && (
                          <Flex css={{ gap: '$2', mt: 10 }} justify="center">
                            ...
                            <FormatCryptoCurrency amount={balance?.value} symbol={balance?.symbol} />
                          </Flex>
                        )}
                        <Flex
                          css={{
                        ...
                        >
                          ...
                          {canAutomaticallyConvert && (
                            <Button css={{ flex: 1, maxHeight: 44 }} disabled={!hasEnoughNativeCurrency} onClick={() => placeBid()}>
                              <Text style="h6" color="button" ellipsify>
                                {copy.ctaConvertAutomatically.length > 0
                                  ? copy.ctaConvertAutomatically
                                  : `Convert ${amountToWrap} ${balance?.symbol || 'ETH'} for me`}
                              </Text>
                            </Button>
                          )}
                        </Flex>
            ...
            {bidStep === BidStep.Offering && collection && (
              ...
              >
                <TransactionBidDetails token={token ? token : undefined} collection={collection} bidData={bidData} />
                <MainContainer css={{ p: '$4' }}>
                  <ProgressBar value={stepData?.stepProgress || 0} max={stepData?.totalSteps || 0} />
                  {transactionError && <ErrorWell css={{ mt: 24 }} />}
                  {stepData && (
                    <>
                      <Text css={{ textAlign: 'center', mt: 48, mb: 28 }} style="subtitle1">
                        {stepTitle}
                      </Text>
                      {stepData.currentStep.kind === 'signature' && (
                        <TransactionProgress justify="center" fromImg={itemImage || ''} toImgs={[localMarketplace?.icon || '']} />
                      )}
                      {stepData.currentStep.kind !== 'signature' && (
                        <Flex align="center" justify="center">
                          ...
                          >
                            <CryptoCurrencyIcon css={{ height: 56, width: 56 }} address={wrappedContractAddress} />
                          </Flex>
                        </Flex>
                      )}
                      <Text
                  ...
                  {transactionError && (
                    <Flex css={{ mt: 'auto', gap: 10 }}>
                      <Button color="secondary" css={{ flex: 1 }} onClick={() => setBidStep(BidStep.SetPrice)}>
                        {copy.ctaEditOffer}
                      </Button>
            ...
            {bidStep === BidStep.Complete && (
              <Flex direction="column" align="center" css={{ p: '$4' }}>
                <Box css={{ color: '$successAccent', mt: 48 }}>
                  <FontAwesomeIcon icon={faCheckCircle} style={{ width: '32px', height: '32px' }} />
                </Box>

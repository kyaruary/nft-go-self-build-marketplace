...
import { Button, Flex, FormatCryptoCurrency, FormatCurrency, Text, QuantitySelector } from '@/primitives';
...
const CartItem: FC<Props> = ({ item, usdConversion, tokenUrl }) => {
  ...
  const { remove, data: cart, setQuantity } = useCart();
  const { currency: cartCurrency, chain: cartChain } = cart;
  ...
  const handleClick = () => {
    let chain = Object.values(allChains).find((chain) => cartChain?.id === chain.id);

    // if (!chain) {
    //   chain = Object.values(customChains).find((chain) => chain.id === (reservoirChain?.id || 1));
    // }
    ...
    if (url) {
      window.location.href = url.replace('${contract}', contract).replace('${tokenId}', token.id);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Flex direction="column" justify="center" css={{ gap: 2, ml: '$2', minWidth: 0 }}>
        ...
        {price ? (
          ...
          >
            ...
            />
            {usdPrice && usdPrice > 0 ? <FormatCurrency amount={usdPrice} style="tiny" color="subtle" css={{ textAlign: 'end' }} /> : null}
          </Flex>
        ) : null}
      </Flex>
      {order && order?.quantityRemaining > 1 && price ? (
        <Flex justify="between" align="center" css={{ width: '100%', px: 24, pb: 8, gap: '$3' }}>
          <Flex direction="column" align="start" css={{ gap: '$1', overflow: 'hidden', minWidth: 0 }}>
            <Text style="body3">Quantity</Text>
            <Text style="body3" color="subtle" ellipsify css={{ width: '100%' }}>

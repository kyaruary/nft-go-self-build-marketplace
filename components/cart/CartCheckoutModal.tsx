...
import { AnimatedOverlay, StyledAnimatedContent, Anchor, Button, Flex, Text, Box, Loader } from "@/primitives";

import { styled } from "@stitches/react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faCheckCircle, faCircleExclamation, faClose, faCube, faLock, faWallet } from "@fortawesome/free-solid-svg-icons";
...
export function CartCheckoutModal({ items, totalPrice, usdPrice, currency, cartChain, blockExplorerBaseUrl, transaction, open, setCartPopoverOpen }: Props): ReactElement | null {
  ...
  const pathMap = transaction?.path
    ? (transaction.path as Path[]).reduce((paths: Record<string, Path>, path: Path) => {
        if (path.orderId) {
              ...
              <Box css={{ maxHeight: "85vh", overflowY: "auto" }}>
                {transaction?.status === CheckoutStatus.Approving && (
                  <Flex direction="column">
                    ...
                    >
                      <TokenCheckout itemCount={items.length} images={images} totalPrice={totalPrice} usdPrice={usdPrice} currency={currency} chain={cartChain} />
                    </Box>
                    <Flex direction="column" css={{ p: "$4", overflowY: "auto" }}>
                      ...
                      >
                        ...
                        {transaction?.currentStep && transaction?.currentStep?.id === "auth" ? <SigninStep css={{ mt: 48, mb: "$4", gap: 20 }} /> : null}
                        {transaction?.currentStep && transaction?.currentStep?.id !== "auth" ? (
                          <>
                            {transaction?.currentStep?.items && transaction.currentStep?.items.length > 1 ? (
                              <Flex direction="column" css={{ gap: "$4", width: "100%" }}>
                                <Text style="h6" css={{ textAlign: "center" }}>
                                  Approve Purchases
                                </Text>
                                <Text style="subtitle2" color="subtle">
                                  Due to limitations with Blur, the purchase of these items needs to be split into {transaction?.currentStep?.items.length} separate transactions.
                                </Text>
                                {transaction.currentStep?.items.map((item) => (
                                  <ApprovePurchasingCollapsible item={item} pathMap={pathMap} usdPrice={usdPrice} chain={cartChain} open={true} />
                                ))}
                              </Flex>
                            ) : (
                              <Flex direction="column" align="center" css={{ gap: "$4", py: "$4" }}>
                    ...
                    <Flex direction="column" css={{ px: "$4", py: "$5" }}>
                      ...
                      >
                        ...
                        <Text style="subtitle2" color="subtle" css={{ textAlign: "center" }}>
                          You can close this modal while it finalizes on the blockchain. The transaction will continue in the background.
                        </Text>

                        <FontAwesomeIcon icon={faCube} style={{ height: 24, width: 24 }} />
                      </Flex>
                    </Flex>
                    <Button disabled={true} css={{ m: "$4" }}>
                      <Loader />
                ...
                {transaction?.status === CheckoutStatus.Complete && (
                  <Flex direction="column" align="center" css={{ width: "100%", p: "$4" }}>
                    <Flex direction="column" align="center" css={{ width: "100%", p: "$4" }}>
                      <Flex direction="column" align="center" css={{ px: "$4", py: "$5", gap: 24 }}>
                        <Box
                          css={{
                            color: failedSales ? "$errorAccent" : "$successAccent",
                          }}
                        >
                          <FontAwesomeIcon icon={failedSales ? faCircleExclamation : faCheckCircle} fontSize={32} />
                        </Box>
                        <Text style="h5" css={{ textAlign: "center" }}>
                          {failedSales ? `${successfulSales} ${successfulSales > 1 ? "items" : "item"} purchased, ${failedSales} ${failedSales > 1 ? "items" : "item"} failed` : "Congrats! Purchase was successful."}
                        </Text>
                        <Flex direction="column" css={{ gap: "$2", mb: "$3" }}>
                          {transaction.currentStep?.items?.map((item) => {
                            const txHash = item.txHash ? `${item.txHash.slice(0, 4)}...${item.txHash.slice(-4)}` : "";
                            return (
                              <Anchor href={`${blockExplorerBaseUrl}/tx/${item?.txHash}`} color="primary" weight="medium" target="_blank" css={{ fontSize: 12 }}>
                    ...
                    <Flex css={{ width: "100%", p: "$4" }}>
                      <Button css={{ width: "100%" }} onClick={() => setDialogOpen(false)}>
                        Close
                      </Button>
                    </Flex>
                  </Flex>

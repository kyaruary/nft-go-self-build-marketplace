...
import { Button, Flex, Text, Loader, Box, Dialog } from '@/primitives';
...
type Props = {
  ...
  onBack?: (() => void) | null;
  loading?: boolean;
} & Pick<ComponentPropsWithoutRef<typeof Dialog>, 'onPointerDownOutside' | 'onOpenChange' | 'open' | 'trigger' | 'onFocusCapture'>;
...
export const Modal = forwardRef<ElementRef<typeof Dialog>, Props>(
  ({ title, children, trigger, onBack, open, size = ModalSize.MD, onOpenChange, loading, onPointerDownOutside, onFocusCapture }, forwardedRef) => {
    return (
      <Dialog
        ref={forwardedRef}
        trigger={trigger}
      ...
      >
        ...
        >
          <Title css={{ alignItems: 'center', display: 'flex' }}>
            {onBack && (
              <Button color="ghost" size="none" css={{ mr: '$2', color: '$neutralText' }} onClick={onBack}>
